...
Внутри каталога проекта имеется каталог `sample-project1-main`. Он является модулем проекта.
...
В данном случае `sample-project1-main.jar`. Это очень важная информация. Именно модуль
...
Строка `moduleDef("sample.project1.main")` определяет модуль Jandcode. 
...
Таким образом, имеем модуль с именем `sample-project1-main`, из которого мы получили библиотеку
с таким же именем `sample-project1-main`. Что, собственно, и является целью модуля -
...
Напомню для начала: модуль при компиляции превращается в библиотеку с именем,
...
Теперь модуль `sample-project1-main` зависит от модуля `sample-project1-module2`. Что кроме
всего прочего означает, что модуль `sample-project1-module2` будет компилироватся
до модуля `sample-project1-main`, а компиляция модуля `sample-project1-main` будет пользоватся
