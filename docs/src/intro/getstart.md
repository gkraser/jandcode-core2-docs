...
!!! note "Создайте алиас для команды ajc"
    Приложение запускается командной `sh ./ajc.sh`, создайте алиас для нее:
    ```shell
    alias ajc='sh ./ajc.sh $*'
    ```
...
В нем есть файл `ajc.bat` (для Linux `ajc.sh`), который управляет запуском 
приложения или его утилит.
